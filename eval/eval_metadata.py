...
dataset_metadata = {
    ...
    'waymo': {
        ...
        'traj_format': None,
        'seq_list': [
            '16345319168590318167_1420_000_1440_000', '183829460855609442_430_000_450_000',
            '3156155872654629090_2474_780_2494_780', '3461811179177118163_1161_000_1181_000',
            '3711598698808133144_2060_000_2080_000', '4058410353286511411_3980_000_4000_000',
            '4604173119409817302_2820_000_2840_000', '5200186706748209867_80_000_100_000',
            '6104545334635651714_2780_000_2800_000'
        ],
        'full_seq': False,
        'mask_path_seq_func': lambda mask_path, seq: None,
        'skip_condition': None,
...
def process_bonn(args, img_path):
    if args.full_seq:
        ...
        seq_list = ["balloon2", "crowd2", "crowd3", "person_tracking2",
                    "synchronous"] if args.seq_list is None else args.seq_list
...
def process_waymo(args, img_path):
    if args.full_seq:
        seq_list = sorted(glob.glob(f'{img_path}/segment-*'))
        for seq in tqdm(seq_list):
            filelist = sorted(glob.glob(f'{seq}/*_1.jpg'))
            save_dir = f'{args.output_dir}/{seq}'
            yield filelist, save_dir
    else:
        seq_list = [
        ...
        for seq in tqdm(seq_list):
            seq_id = f'segment-{seq}_with_camera_labels.tfrecord'
            filelist = sorted(glob.glob(f'{img_path}/{seq_id}/*_1.jpg'))
            save_dir = f'{args.output_dir}/{seq}'
            yield filelist, save_dir
