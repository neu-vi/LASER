...
def eval_mono_depth_estimation(args, model, device, dtype):
    ...
    for filelist, save_dir in process_func(args, img_path):
        ...
        eval_mono_depth(args, model, device, dtype, filelist, save_dir=save_dir)


def eval_mono_depth(args, model, device, dtype, filelist, save_dir=None):
    model.eval()
    for files in filelist:
        # TODO: it's wrong as the input is not a list of images
        ...
        depth_maps = predictions["depth"]  # (S, H, W, 1)
        for file, depth_map in tqdm(zip(files, depth_maps)):
            if save_dir is not None:
                #save the depth map to the save_dir as npy
                np.save(f"{save_dir}/{file.split('/')[-1].replace('.png','depth.npy')}", depth_map.cpu().numpy())
                # also save the png
                depth_map = (depth_map - depth_map.min()) / (depth_map.max() - depth_map.min())
                depth_map = (depth_map * 255).cpu().numpy().astype(np.uint8)
                cv2.imwrite(f"{save_dir}/{file.split('/')[-1].replace('.png','depth.png')}", depth_map)
