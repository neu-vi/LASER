...
class Block(nn.Module):
    def __init__(
        ...
        attn_class: Callable[..., nn.Module] = EfficientAttention,
    ...
    def forward(self, x: Tensor, pos=None, return_attn=False, S=None) -> Tensor:
        def attn_residual_func(x: Tensor, pos=None, return_attn=False, S=None) -> Tensor:
            if not return_attn:
                return self.ls1(self.attn(self.norm1(x), pos=pos, S=S))
            x, attn_logit = self.attn(self.norm1(x), pos=pos, return_attn=True, S=S)
            return self.ls1(x), attn_logit
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            x = x + self.drop_path1(attn_residual_func(x, pos=pos, S=S))
            x = x + self.drop_path1(ffn_residual_func(x))  # FIXME: drop_path2
        elif return_attn:
            x_, attn_logit = attn_residual_func(x, pos=pos, return_attn=True)
            x = x + self.drop_path1(x_)
            x = x + self.drop_path1(ffn_residual_func(x))
            return x, attn_logit
        else:
            x = x + attn_residual_func(x, pos=pos, S=S)
