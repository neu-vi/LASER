...
def depth_to_cam_coords_points_batch(depth: torch.Tensor, intrinsic: torch.Tensor) -> torch.Tensor:
    """
    Convert a depth map (or batch of depth maps) to camera coordinates.

    Args:
        depth (torch.Tensor): Depth maps of shape (..., H, W).
        intrinsic (torch.Tensor): Camera intrinsics of shape (..., 3, 3).

    Returns:
        torch.Tensor: Camera coordinates of shape (..., H, W, 3).
    """
    *batch_dims, H, W = depth.shape
    assert intrinsic.shape[-2:] == (3, 3), "Intrinsic matrix must be (..., 3, 3)"

    device, dtype = depth.device, depth.dtype

    # Intrinsic parameters
    fu = intrinsic[..., 0, 0][..., None, None]
    fv = intrinsic[..., 1, 1][..., None, None]
    cu = intrinsic[..., 0, 2][..., None, None]
    cv = intrinsic[..., 1, 2][..., None, None]
    ...
    # Broadcast to batch
    u = u.expand(*batch_dims, H, W)
    v = v.expand(*batch_dims, H, W)
