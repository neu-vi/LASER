        ...
        if osp.exists(cache_file):
            ...
            for seq in tqdm(self.sequence_list):
                ...
                if not (len(seq_rgbs) == len(seq_depths) == len(seq_poses)):
                    raise ValueError(
                        f"Sequence {seq} has mismatched number of RGB, depth, and pose files: {len(seq_rgbs)}, {len(seq_depths)}, {len(seq_poses)}")
                get_number_func = lambda x: int(x.split('-')[1].split('.')[0])
                if not (get_number_func(seq_rgbs[0]) == get_number_func(seq_depths[0]) == get_number_func(
                        seq_poses[0]) == 0):
                    raise ValueError(
                        f"First frame of sequence {seq} has number of RGB, depth, and pose != 0: {seq_rgbs[0]}, {seq_depths[0]}, {seq_poses[0]}")
                if not (get_number_func(seq_rgbs[-1]) == get_number_func(seq_depths[-1]) == get_number_func(
                        seq_poses[-1]) == len(seq_rgbs) - 1):
                    raise ValueError(
                        f"Last frame of sequence {seq} has number of RGB, depth, and pose != N-1: {seq_rgbs[-1]}, {seq_depths[-1]}, {seq_poses[-1]}, N={len(seq_rgbs)}")
