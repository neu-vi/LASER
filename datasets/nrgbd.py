    ...
    def __init__(
        ...
        if osp.exists(cache_file):
            ...
            for seq in tqdm(self.sequence_list):
                ...
                if not (int(seq_rgbs[0].split('.')[0][len('img'):]) == int(
                        seq_depths[0].split('.')[0][len('depth'):]) == 0):
                    raise ValueError(
                        f"First frame of sequence {seq} has number of RGB and depth != 0: {seq_rgbs[0]}, {seq_depths[0]}, {seq_poses[0]}")
                if not (int(seq_rgbs[-1].split('.')[0][len('img'):]) == int(
                        seq_depths[-1].split('.')[0][len('depth'):]) == len(seq_rgbs) - 1):
                    raise ValueError(
                        f"Last frame of sequence {seq} has number of RGB and depth != N-1: {seq_rgbs[-1]}, {seq_depths[-1]}, N={len(seq_rgbs)}")
