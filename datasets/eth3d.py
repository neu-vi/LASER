...
class ETH3D(Dataset):
    ...
    def get_data(
        ...
        for id_index, id in enumerate(ids):
            ...
            depthpath = os.path.join(self.ETH3D_DIR, sequence_name, 'ground_truth_depth', 'custom_undistorted',
                                     img_name)
            cam_path = os.path.join(self.ETH3D_DIR, sequence_name, 'custom_undistorted_cam',
                                    img_name.replace('JPG', 'npz'))
